...
processed = None
path = None
moving = "DEFAULT"          #nacin kretanja robota
...
def do_search():
    ...
    start = time.clock()
    path, processed, states = search.search(initial_state, moving)
    end = time.clock()
...
box = ttk.Combobox(ui, textvariable="")
box['values'] = ('DEFAULT', 'KNIGHT', 'QUEEN', 'ROOK')
box.current(0)
...
box.grid(row=6, column=0, padx=10, pady=10)
